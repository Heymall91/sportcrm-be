...
export class ClubStaffController {
  ...
  @ApiParam({name: "id", description: "Club staff identificator", example: "6505ad1f-03d0-43b7-9d85-44404f7cdbf2"})
  @ApiResponse({status: 200, description: "Club staff"})
  @ApiResponse({status: 404, description: "Relation doesn't exist"})
  async findOne(@Param('id', new ParseUUIDPipe()) id: string): Promise<CreateClubStaffDto> {
  ...
  @ApiParam({name: "id", description: "Club staff identificator", example: "6505ad1f-03d0-43b7-9d85-44404f7cdbf2"})
  @ApiResponse({status: 200, description: "Club staff updated", type: CreateClubStaffDto})
  @ApiResponse({status: 404, description: "Relation doesn't exist"})
  async update(@Param('id', new ParseUUIDPipe()) id: string, @Body() updateClubStaffDto: UpdateClubStaffDto): Promise<UpdateClubStaffDto> {
    const updatedClubStaffDto = await this.clubStaffService.update(id, updateClubStaffDto);
    return plainToInstance(UpdateClubStaffDto, updatedClubStaffDto);
  }

  @Delete(':id')
  @ApiOperation({summary: "Deleting club staff by ID"})
  @ApiParam({ name: 'id', description: "Club staff identificator", example: "c5d9367e-f6b4-41b0-a8e3-fe8d9c4e31ba"})
